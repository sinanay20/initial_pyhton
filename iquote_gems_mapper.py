...
root = Tk()
config_path = './config/iquote_gems_mapper.yml'
input_file_path = ""
role_mapping = []
...
def create_gems_file(user_list):
    ...
    for user in user_list:
        if user[2] is not None:
            ...
            for product in product_profile1:
                if user[2] == product[1]:
                    out_list.append([user[0], 'IQUOTE_PRODUCTPROFILE1', '', user[2], '', ''])

        for role in user[1]:
            out_list.append([user[0], role, '', '', '', ''])
...
def parse_iquote_file():
    ...
    with open(input_file_path, 'rb')as input_file:
        ...
        for i, line in enumerate(input_reader):
            ...
            # org_scope = decode_orgs(line[25])
            user_list.append([line[0], roles, profile])
    return user_list
...
def read_config():
    ...
    try:
        ...
        for profile in config["product_profile1"]:
            complete_product_profile.append([profile, config["product_profile1"][profile]])
            product_profile1.append([profile, config["product_profile1"][profile]])
